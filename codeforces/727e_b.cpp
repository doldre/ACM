...
pair<ull, ull> cal_hash(string s) {
    ...
    for (int i = 0; i < len; i++) {
        _hash1 = _hash1 * seed1 + s[i];
        _hash2 = _hash2 * seed2 + s[i];
    }
...
void pre_solve() {
    ...
    ull _hash1 = 0, _hash2 = 0;
    for (int i = 0; i < k; i++) {
        _hash1 = _hash1 * seed1 + str[i];
        _hash2 = _hash2 * seed2 + str[i];
    }
    ...
    for (int i = 0; i < n * k; i++) {
        ...
        _hash1 = (_hash1 - C1 * str[i]) * seed1 + str[(i+k) % str.size()];
        _hash2 = (_hash2 - C2 * str[i]) * seed2 + str[(i+k) % str.size()];
    }
}
